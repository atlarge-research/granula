...
import science.atlarge.granula.modeller.platform.info.*;
    ...
    public boolean execute() {
        ...
        for (Operation suboperation : operation.getChildren()) {
            if(suboperation.getMission().getType().equals(missionType)) {
                startTimeInfos.add(suboperation.getInfo(BasicType.StartTime));
                endTimeInfos.add(suboperation.getInfo(BasicType.EndTime));
