...
function operChart(operation) {
    ...
    supActorIds = subActors.map(function (actor) {return actor.uuid;});

    var bgWidth = 1000;
    // var bgHeight = (subActors.length > 3) ? 65+50* subActors.length : 65+50*3;
    var bgHeight = 65+40* subActors.length;
    ...
    var bgRect = rect(board, 0, 0, bgWidth, bgHeight, 5, "#FFF");
    ...
    var explRect = {x: 20, y:20, w: 200, h:40};
    ...
    subActors.forEach(function (actor) {
        var actorIndex = supActorIds.indexOf(actor.uuid);
        subActor(board, explRect, actor, actorIndex);
    });
...
function axislines(board, mRect, operation) {
    ...
    for (var i = 0; i <= ticsSize; i++) {
        svgLine(board, mRect.x + distance * i, mRect.y, mRect.x + distance * i, mRect.y+5);
        svgLine(board, mRect.x + distance * i, mRect.y + mRect.h, mRect.x + distance * i, mRect.y + mRect.h - 5);
...
function subActor(board, explRect, actor, i) {


    var x = explRect.x + 10;
    var x2 = explRect.w + explRect.x - 20;
    var sRect = {x: x, y:explRect.y + 10 +i*40, w: (x2 - x), h:(35)};

    var subOperRect;
    subOperRect = rect(board, sRect.x, sRect.y, sRect.w, sRect.h, 2, "#EEE");
    subOperRect.attr({strokeWidth: 1, stroke: change_brightness("#EEE", -80)});


    // var hint = getOperationName(subOper) + " [" + info(subOper, "Duration")  + " ms]";
    // subOperRect.append(Snap.parse('<title>' + hint + '</title>'));
    svgText(board, actor.name, sRect, 18, "#000");

    subOperRect.attr({uuid: actor.uuid});
...
function subOperation(board, mRect, startTime, endTime, subOper, i) {
    ...
    var sRect = {x: x, y:mRect.y + 10 +i*40, w: (x2 - x), h:(35)};

    var blueColor = "#39F";
    var redColor = "#F63";

    try {
        var givenColor = (info(subOper, "Color"));
    } catch (e) {
    ...
    var subOperRect;
    subOperRect = rect(board, sRect.x, sRect.y, sRect.w, sRect.h, 0, color);
    subOperRect.attr({strokeWidth: 1, stroke: change_brightness(color, -80)});
...
function actorList(actorIds) {
    ...
    function compareMethod (actor) {return actor.name};

    return _.sortBy(_.unique(listMethod(actorIds), equalMethod), compareMethod);
}
//
