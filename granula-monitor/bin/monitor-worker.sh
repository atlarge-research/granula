...
SERVICE_NAME="Granula monitor-worker"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PATH_TO_JAR=$SCRIPT_DIR/../target/granula-monitor-1.0-SNAPSHOT.jar
MAIN_CLASS=science.atlarge.granula.monitor.executor.GranulaMonitorWorker
PNAME=GranulaMonitorWorker
PID_PATH_NAME=$SCRIPT_DIR/../log/`hostname`-worker.pid
LOG_FILE=$SCRIPT_DIR/../log/`hostname`-worker.log
